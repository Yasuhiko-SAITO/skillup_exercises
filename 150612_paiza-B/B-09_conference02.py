...
for rep in range(ppt_num):
    ...
    # お昼休憩を入れるかどうか判別。
    # ランチ回数が0、時間が12時以降で、ただし12：00は除く。
    # この時、60分休憩にするため、10分休憩に50分休憩を加える
    hoge_d = d + datetime.timedelta(minutes = add_min)
    if (lunch_count == 0) and (hoge_d.hour >= 12) and \
     not((hoge_d.hour == 12) and (hoge_d.minute == 0)):
    ...
    ans_list.append(start_time.strftime("%H:%M") + " - " + \
                    end_time.strftime("%H:%M") + " " + ppt_list[rep][0])
