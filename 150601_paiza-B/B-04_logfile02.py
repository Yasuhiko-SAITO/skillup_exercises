...
if target_ip_list[2] == "*":        # *が入力された時
    ...
    target_ip_list[2] = hoge_3_range[third_octed_range_first :]
else:   # 数字が入力された時
...
if target_ip_list[3] == "*":
    ...
    fourth_octed_range_first = int(hoge_tor[0][1:])
    fourth_octed_range_last = int(hoge_tor[1][:len(hoge_tor[1])-1])
    hoge_4_range = range(fourth_octed_range_last + 1)
    target_ip_list[3] = hoge_4_range[fourth_octed_range_first :]
else:
...
for i in range(inp_num):
    ...
    if int(check_ip_list[0]) != int(target_ip_list[0]):
        continue
    if int(check_ip_list[0]) == int(target_ip_list[0]):
        hit += 1

    if int(check_ip_list[1]) != int(target_ip_list[1]):
        continue
    if int(check_ip_list[1]) == int(target_ip_list[1]):
        hit += 1
